 ...
 * 定时器服务，需要添加到spring容器
 * 
 * 结合 TimePlanService，可以从数据库读取任务计划
 * 
 * <p>
 * 
 * @author syz
 *         <p>
 * @date 2015-9-11,下午03:14:49
 *       <p>
 * @version v1.0.0
 *          <p>
 * @see com.gbcom.system.timer.service.SysTimerService
 */
public class SysTimerService {
	 ...
	 * 定时器，定期检查
	 */
	private final Timer checkServer;
	...
	public void start() {
		...
		checkServer.schedule(new TimerTask() {
			...
			public void run() {
				...
				// 检查定时任务
				List<TimePlan> list = timePlanService.findByProperty("state",
						true);
				if (list.size() > 0) {
					int i = 0;
					for (i = 0; i < list.size(); i++) {
						...
						int interval = timePlan.getIntervalTime(); // 分钟
						int index = 0;
						int offset = 0;
						...
						case TimePlan.TIME_PLAN_TYPE_ONE:
							...
							// timePlan.getSec());
							if (timePlan.getYear() == year
									&& timePlan.getMin() == month
									&& timePlan.getDay() == day
									&& timePlan.getHour() == hour
									&& timePlan.getMin() == minute) {
						...
						case TimePlan.TIME_PLAN_TYPE_MINUTE:
							...
							cal.set(timePlan.getYear(),
									timePlan.getMonth() - 1, timePlan.getDay(),
									timePlan.getHour(), timePlan.getMin(), 0);
							...
							long tabMinutes = (calendarTime - calTime)
									/ (1000 * 60);
