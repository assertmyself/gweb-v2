 ...
 * @author syz
 * <p>
 * @date 2015-9-11,下午02:30:06
 * <p>
 * @version v1.0.0
 * <p>
 * @see com.gbcom.system.utils.FileUtils
 */
public class FileUtils {
	 ...
	 * DENIED_FILE_EXT
	 */
	public static final String DENIED_FILE_EXT = "exe,bat,jsp,htm,html,js,xml,sql,asp,aspx,dll,cs"; // 禁止上传文件类型
	...
	public static boolean checkFileExt(String fileName) {
		...
		if (!StringHelper.isEmpty(fileExt)) {
			return ("," + DENIED_FILE_EXT + ",").indexOf(","
	 ...
	 * 
	 * @param fileName
	 *            可以为网络地址，比如http://xxx/a.jpg，也可以上fckeditor上传的图片地址，比如/mis/userfiles
	 *            /image/00079166.jpg
	 * @param ctx
	 *            图片路径
	 * @return 是否存在
	...
	public static void downloadSection(HttpServletRequest request,
		...
		if (p != 0) {
			...
			response.setHeader("Content-Range", "bytes "
					+ new Long(p).toString() + "-" + new Long(l - 1).toString()
					+ "/" + new Long(l).toString());
		}
	...
	public static void setDownloadContentType(HttpServletRequest request,
		...
		if (agent == null) {
			...
			if (isMozilla) { // 浏览器下载
				response
						.setContentType("application/x-act-msdownload;charset=UTF-8");
			} else {
				response.setContentType("APPLICATION/OCTET-STREAM");
			}
