 ...
 * @author syz
 *         <p>
 * @date 2015-5-23,上午09:51:44
 *       <p>
 * @version v1.0.0
 *          <p>
 * @see com.gbcom.system.controller.SysAreaController
 */
@Controller
public class SysAreaController extends BaseCRUDActionController<SysArea> {
	private static final Logger LOG = LoggerFactory
			.getLogger(SysAreaController.class);
	@Autowired
	...
	public String tree(Model model) {
		// 判断是否有编辑权限
		model.addAttribute("canEdit", sysUserManager
				.hasPrivilege(PrivilegeCode.SYS_AREA_EDIT));
	...
	public void gridDataCustom(HttpServletResponse response, String filters,
			String columns, int page, int rows, Long codeId, HttpSession session) {
		try {
			Page<SysArea> pageModel = new Page<SysArea>(page, rows, true);
			String hql = "from SysArea order by id ";
			if (codeId != null) {
				hql = "from SysArea where parent.id=" + codeId
						+ " order by id ASC";
			}
			// 增加自定义查询条件
	...
	public String treeData(String type, String id, String icon, Model model) {
		...
		if (StringUtils.isEmpty(id)) {
			...
			String hql = "from SysArea where parent.id=" + id
					+ " order by id asc";
			List<SysArea> nodeList = sysAreaService.findByQuery(hql);
			for (SysArea data : nodeList) {
				boolean isLeaf = data.getIsLeaf();
	...
	/**
	 * : (SysAreaController.treeDataForSelect)
	 * @param code String
	 * @param type String
	 * @param id String
	 * @param icon String
	...
	@RequestMapping
	@Deprecated
	public String treeDataForSelect(String code, String type, String id,
			String icon, Model model) {//select.js
		try {
			SysUser sysuser = sysUserManager.getSysUser();
			int roleType = 0;// 0:系统级 1：商业wifi 2：区域
			for (SysUserRole role : sysuser.getSysUserRoles()) { // 角色
				// 为多对多会有问题，暂时只考虑多对一情况。
			...
			if (StringUtils.isEmpty(id)) {
				...
				String hql = "";
				if (roleType == 2) {// 如果是区域用户，新增用户只能够选择，该区域用户管理的区域的子区域
					SysArea area = sysuser.getArea();
					hql = "from SysArea where  parent=" + area.getId()
							+ " order by id asc";
				} else {
	...
	public void save(HttpServletRequest request, HttpServletResponse response,
			String parentId, @ModelAttribute("bean") SysArea entity)
			throws Exception {
		try {
			SysArea target;
	...
	public String setMngNes(Model model, Long id) {
		...
		// 按钮权限
		SysCodeDetail privilegeType = sysCodeManager.getCodeDetailByCode(
				Constants.SYS_PRIVILEGE_TYPE,
				Constants.SYS_PRIVILEGE_TYPE_BUTTON);
	...
	public void saveMngNes(HttpServletResponse response, Long id,
	 ...
	 */
	@RequestMapping
	public void setUnGroupNes(HttpServletResponse response,
			HttpServletRequest request) {
		try {
			String[] ids = request.getParameterValues("mngNesId");
	...
	public String searhMngNes(Model model, Long id, String sgwId) {
		...
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		List<String> groupIDS = sysAreaNesService.getAllHots();
		for (String gw : apDeviceManager.getSessionHotKeys()) {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("gwId", gw);
			boolean isChecked = idSet.contains(gw);
			map.put("isChecked", isChecked);
			if (!isChecked) {
				if (groupIDS.contains(gw)) {
					continue;
				}
			}
			Hotspot hotspot = GwifiCacher.getInstance().getGwHotspot(gw);
			StringBuffer name = new StringBuffer("");
			if (hotspot == null) {
				if (gw.contains(sgwId)||isChecked) {
					name.append(gw);
				}
			} else {
				if (hotspot.getStreet() == null
						|| hotspot.getStreet().equals("")) {
					if ((hotspot.getName() + " / " + hotspot.getGwID())
							.contains(sgwId)||isChecked) {
						name.append(hotspot.getName() + " / "
								+ hotspot.getGwID());
					}
				} else {
					if ((hotspot.getName() + " / " + hotspot.getGwID() + " / " + hotspot
							.getStreet()).contains(sgwId)||isChecked) {
						name.append(hotspot.getName() + " / "
								+ hotspot.getGwID() + " / "
								+ hotspot.getStreet());
					}
				...
				List<ApDevice> aps = apDeviceManager.findByHot(gw);
				if(aps.size()>0){
					for(ApDevice dev : aps ){
						if(String.valueOf(dev.getSwVersion()+"/"+dev.getManageIP()).contains(sgwId)){
							if(name.toString().equals("")){
								name.append(gw+"/"+dev.getSwVersion()+"/"+dev.getManageIP());
							}else{
								name.append("/"+dev.getSwVersion()+"/"+dev.getManageIP());
							}
							break;
						}
					}
...
//			nameAppend(name, eachMap.get("manage_mac"));
	...
	/*
	private List<Map<String,Object>> findChecked(SysArea sysArea,String sgwId){
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		
		Set<SysAreaNes> nes = sysArea.getNes();
		for (SysAreaNes ne : nes) {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("gwId", ne.getNeID());
			map.put("isChecked", true);
			
			Hotspot hotspot = GwifiCacher.getInstance().getGwHotspot(ne.getNeID());
			StringBuffer name = new StringBuffer("");
			if (hotspot == null) {
				name.append(ne.getNeID());
			} else {
				if (hotspot.getStreet() == null
						|| hotspot.getStreet().equals("")) {
						name.append(hotspot.getName() + " / "
	...
	*/
	...
	private void nameAppend(StringBuffer rst,Object appended){
		if(appended !=null){
	 ...
	 * @return .
	 */
	private List<Map<String, Object>> queryRight(String replace) {
		if(StringUtils.isEmpty(replace)){
			return new ArrayList<Map<String, Object>>(0);
		}
		String sql = "SELECT b.gw_id,a.name ,a.street_address,b.manage_mac ,b.ap_name ,b.sys_model,  b.manageip, b.sw_version " +
				"FROM v3_hotspot AS a  RIGHT JOIN v3_apdevice AS b ON a.gw_id=b.gw_id " +
				"WHERE  b.gw_id LIKE '%sgwId%' OR a.name LIKE '%sgwId%' OR a.street_address LIKE '%sgwId%' OR b.manage_mac LIKE '%sgwId%' " +
				"OR b.ap_name  LIKE '%sgwId%' OR b.sys_model LIKE '%sgwId%' OR  b.manageip LIKE '%sgwId%' OR b.sw_version LIKE '%sgwId%' " +
				"GROUP BY b.gw_id";
