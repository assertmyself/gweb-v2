...
public class SysOnlineUserController extends BaseCRUDActionController<SysUser> {
	...
	public void gridDataCustom(HttpServletResponse response, String filters,
			String columns, int page, int rows, HttpSession session,String userName,String realName) {
		try {
			...
			for (Object ob : countUsers) {
				...
				if (!userNames.contains(baseUser.getUsername())) {
					...
					if (sysUser != null) {
						String sql = "select ip_address from sys_log where enter_time=(select max(enter_time) from sys_log where user_id="
								+ sysUser.getId() + ") and user_id=" + sysUser.getId();
						...
						if(sysUser.getPerson()!=null){
							if(sysUser.getPerson().getCompany()!=null){
								String dept = sysUser.getPerson().getCompany()
								.getName();
					...
					cellMap.put("onlineTimeMinute",
							(System.currentTimeMillis() - baseUser
									.getLoginTime().getTime()) / 60000);
	...
	@RequestMapping
    @UserLog
	public String view(Model model,@RequestParam(value="username") String username){
		List countUsers = sessionRegistry.getAllPrincipals();
		String userName = "";
		String realName = "";
		String loginTime = "";
		...
		for (Object ob : countUsers) {
			...
			if(baseUser.getUsername().equals(username)){
				...
				SysUser sysUser = sysUserManager.getSysUser(baseUser
						.getLoginName());
				if (sysUser != null) {
					String sql = "select ip_address from sys_log where enter_time=(select max(enter_time) from sys_log where user_id="
							+ sysUser.getId()
							+ ") and user_id="
							+ sysUser.getId();
					ipAddress = simpleQueryManager
							.getStringBySql(sql);
					
					if(sysUser.getPerson()!=null){
						if(sysUser.getPerson().getCompany()!=null){
							dept = sysUser.getPerson().getCompany()
							.getName();
						}
					}
