...
public class SysMenuManager {
	...
	public String getUserMenuJson(Long userId) {
		...
		Set<String> privileges = sysUserManager
				.getUserPrivilegeCodesCache(userId);
		List<Map<String, Object>> menuList = getMenuList();
		for (Map<String, Object> menuItem : menuList) {
			List<Map<String, Object>> childMenuItems = getChildMenuItem(
					menuItem, privileges);
			if (childMenuItems != null) {
				menuItem.put("menus", childMenuItems);
				retList.add(menuItem);
			} else if (privileges.contains(JspHelper.getString(menuItem
					.get("privilege")))) {
				menuItem.put("menus", "");
				retList.add(menuItem);
			}
	...
	private List<Map<String, Object>> getChildMenuItem(
			Map<String, Object> menuItem, Set<String> privilegeMap) {
		List<Map<String, Object>> ret = new ArrayList<Map<String, Object>>();

		List<Map<String, Object>> itemList = (List<Map<String, Object>>) menuItem
				.get("menus");
		if (itemList != null) {
			for (Map<String, Object> item : itemList) {
				if (item.get("menus") != null) {
					...
					ret.add(item);
				} else if (privilegeMap.contains(JspHelper.getString(item
						.get("privilege")))) {
					item.put("menus", "");
					ret.add(item);
