 ...
 */
@Controller
public class AutoUpdateServerController extends
	...
	private static final Logger LOG = Logger
			.getLogger(AutoUpdateServerController.class);
	@Autowired
	...
	public void checkedVer(HttpServletRequest request,
			HttpServletResponse response, String product, String curVer,
			String verNo, String time) {
		try {
			boolean result = autoUpgradeServerService.isNewestVersion(product,
					curVer, verNo, time);
			if (result) {
				sendJSON(response, JsonUtil.beanToJSON(null));
			} else {
				VersionInfo versionInfo = UpdateServerContextManager
			...
			sysLogManager.log("admin", request.getRequestURI(), request
					...
					UserLog.USERLOG_LOGTYPE_BUSSINESS, "AutoUpdateServer",
					UserLog.USERLOG_EVENTTYPE_UPDATE, "检查版本");
	...
	public void downloadVer(HttpServletRequest request,
			HttpServletResponse response, String product) {
		try {
			autoUpgradeServerService.downNewestVersion(response, request,
					product);
			sysLogManager.log("admin", request.getRequestURI(), request
					.getRemoteHost(), request.getRequestedSessionId(),
					UserLog.USERLOG_LOGTYPE_BUSSINESS, "AutoUpdateServer",
					UserLog.USERLOG_EVENTTYPE_UPDATE, "下载版本");
		} catch (AutoUpdateException e) {
			LOG.error(e.getMessage(), e);
		}
	}
