 ...
 */
@Controller
public class AutoUpdateClientController extends
		BaseCRUDActionController<Serializable> {
	/**
	 * 日志记录器
	 */
	private static final Logger LOG = Logger.getLogger(AutoUpdateClientController.class);
	@Autowired
	...
	public void autoUpdate(HttpServletRequest request,
		...
		try {
			...
			// 2.下载最新版本
			autoUpdateClientService.downNewestVersion(request, versionInfo
					.getProduct());
			// 升级
			String updateMethod = UpdateClientContextManager.getInstance()
					.getVersionInfo(versionInfo.getProduct()).getMethod();
			...
			if (updateMethod.equals("sh")) {
				...
				String proRealPath = request.getSession().getServletContext()
						.getRealPath("");
				autoUpdateClientService.codeUpdate(proRealPath);
				sendSuccessJSON(response, "升级成功，稍候将重启服务器！");
			}
			sysLogManager.log("admin", request.getRequestURI(), request
					.getRemoteHost(), request.getRequestedSessionId(),
					UserLog.USERLOG_LOGTYPE_BUSSINESS, "AutoUpdateClient",
					UserLog.USERLOG_EVENTTYPE_UPDATE, "升级web版本");
		} catch (Exception e) {
	 ...
	 */
	@RequestMapping
	public void restartServer(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			String proRealPath = request.getSession().getServletContext()
					.getRealPath("");
			String tomcatBinPath = proRealPath.substring(0, proRealPath
					.indexOf("webapps"))
					+ "bin";
	...
	public String verCheck(Model model) {
		...
		try {
			...
			String product = UpdateClientContextManager.getInstance()
					.getUpdateProduct();
			curVerInfo = UpdateClientContextManager.getInstance()
	...
	public void updateCheck(HttpServletRequest request,
		...
		try {
			String product = UpdateClientContextManager.getInstance()
					.getUpdateProduct();
			VersionInfo versionInfo = VersionInfoManager.getInstance().get(
					product);
			if (versionInfo == null) {
