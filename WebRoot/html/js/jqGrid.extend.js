...
//    })();
	...
	$("#gview_"+$(ele).attr("id")).css({"overflow-x":"hidden"});//隐藏jqgrid 自带的横向滚动条
    /*================end====初始化grid==============*/
...
/**
 * 对外提供的 调整页面大小的方法，需包含在jqGrid_wrapper div中
 * @return
 */
function resizeGrid(){
	$(".jqGrid_wrapper").css({"overflow-x":"hidden"});
	var width = $(".jqGrid_wrapper").width();
	var ele=$(".jqGrid_wrapper table")[1];
	if(width<600){
		width=600;
		$(".jqGrid_wrapper").css({"overflow-x":"auto"});
	}
	$(ele).setGridWidth(width-10);
}